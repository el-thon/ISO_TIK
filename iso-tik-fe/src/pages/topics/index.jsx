...
export default function TopicsPage() {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } = useTopics(params)
  ...
  } = useTopics(myTopicsParams, { keepPreviousData: true })
      ...
      <div className="max-w-full mx-auto px-6 py-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/topics/create">
              <Button size="sm" className="bg-blue-600 hover:bg-blue-500 text-white flex items-center gap-2">
          ...
          <CardContent className="pt-5">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Pantau cepat 5 topik terbaru yang Anda buat.</p>
              </div>
              <Button variant="outline" size="sm" disabled={myTopicsLoading} onClick={() => refetchMyTopics()}>
                Segarkan
              </Button>
            </div>
            {myTopicsLoading ? (
              ...
              <div className="text-sm text-rose-600 flex items-center justify-between">
                <span>{myTopicsErrorObj?.response?.data?.message || myTopicsErrorObj?.message || 'Gagal memuat data.'}</span>
                <Button variant="outline" size="sm" onClick={() => refetchMyTopics()}>
                  Coba lagi
                </Button>
              </div>
            ) : myTopics.length === 0 ? (
              ...
              <div className="divide-y">
                {myTopics.map((topic) => (
                  <Link key={topic.id} to={`/topics/${topic.id}`} className="flex items-center justify-between py-3 hover:text-primary">
                    <div>
                      <p className="font-medium">{topic.title}</p>
                      <p className="text-xs text-muted-foreground">{formatDate(topic.updated_at)} • {topic.status || 'draft'}</p>
                    </div>
        ...
        {isError && (
          <div className="p-4 mb-6 rounded-md bg-rose-50 border border-rose-100 text-sm text-rose-700 flex items-center justify-between">
            <span>{error?.response?.data?.message || error?.message || 'Gagal memuat data topik.'}</span>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {topics.map((topic) => {
              ...
              const responsibleName = topic.room?.responsible_user?.profile?.full_name || topic.room?.responsible_user?.username
              return (
                <Card key={topic.id}>
                      ...
                      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-2 mt-3 text-xs text-muted-foreground">
                            {topic.room?.name && (
                              <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-700">{topic.room.name}</span>
                            )}
